...
describe('Convert currency tests.', () => {
  it('Throws when converting unsupported currencies', () => {
    ...
    // act and assert
    expect(() => convertCurrency('EUR', 'UNSUPPORTED', 1)).toThrow(UnsupportedCurrencyConversionError);
    expect(() => convertCurrency('UNSUPPORTED', 'EUR', 1)).toThrow(UnsupportedCurrencyConversionError);
  })
